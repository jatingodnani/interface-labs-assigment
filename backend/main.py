...
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql://postgres:hello%40post123@db/labsassigment")
...
@app.post("/upload-both-mtr-paymentfile")
async def upload_both(file: UploadFile = File(...), file2: UploadFile = File(...)):
    try:
        logger.info("Starting file upload and processing")

        # Validate file types
        valid_types = ["text/csv", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"]
        if file.content_type not in valid_types or file2.content_type not in valid_types:
            logger.error("Invalid file types uploaded")
            return JSONResponse(content={"error": "Both files must be either CSV or XLSX"}, status_code=400)

        # Read the first file (Payment Report) into a DataFrame
        contents1 = await file.read()
        df1 = pd.read_csv(io.StringIO(contents1.decode("utf-8"))) if file.content_type == "text/csv" else pd.read_excel(io.BytesIO(contents1))
        logger.info(f"Payment Report loaded. Shape: {df1.shape}")

        # Read the second file (MTR) into another DataFrame
        contents2 = await file2.read()
        df2 = pd.read_csv(io.StringIO(contents2.decode("utf-8"))) if file2.content_type == "text/csv" else pd.read_excel(io.BytesIO(contents2))
        ...
        if df1.empty or df2.empty:
            ...
            return JSONResponse(content={"error": "One of the uploaded files is empty or not properly formatted"}, status_code=400)

        # Standardize column names
        df1.columns = df1.columns.str.strip().str.lower()
        df2.columns = df2.columns.str.strip().str.lower()

        # Process Payment Report (df1)
        if "type" in df1.columns:
            df1 = df1[df1["type"] != "transfer"].rename(columns={"type": "payment type"})
            ...
            return JSONResponse(content={"error": "'type' column not found in the Payment Report"}, status_code=400)

        # Process MTR Report (df2)
        if "transaction type" in df2.columns:
            ...
            return JSONResponse(content={"error": "'transaction type' column not found in the MTR Report"}, status_code=400)
        ...
        # Define SQLAlchemy types for the columns
        sql_dtypes = {
            'order id': types.Text(),
        ...
        # Truncate both tables before inserting new data
        with engine.connect() as connection:
            connection.execute(text(f"TRUNCATE TABLE processed_transactions RESTART IDENTITY CASCADE"))
            connection.execute(text(f"TRUNCATE TABLE summary_table RESTART IDENTITY CASCADE"))

        # Save the processed DataFrame to the database
        table_name = "processed_transactions"
        df_to_insert.to_sql(table_name, engine, if_exists='append', index=False, dtype=sql_dtypes)
        ...
        summary_df.to_sql(summary_table_name, engine, if_exists='append', index=False, dtype=summary_sql_dtypes)
